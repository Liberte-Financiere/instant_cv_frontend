    ...
    <div className="cv-template w-full h-full bg-white text-slate-900 min-h-[297mm] font-sans">
      ...
      <div className="p-10 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-6">
          {qualities.length > 0 && (
            <section>
              <h2 className="text-xs font-black uppercase tracking-widest mb-2" style={{ color: accent }}>{SECTION_TITLES.qualities}</h2>
              {qualities.map((q) => <p key={q.id} className="text-xs text-slate-600 mb-0.5">▸ {q.name}</p>)}
            </section>
          )}
          {languages.length > 0 && (
            <section>
              <h2 className="text-xs font-black uppercase tracking-widest mb-2" style={{ color: accent }}>{SECTION_TITLES.languages}</h2>
              {languages.map((l) => <p key={l.id} className="text-xs text-slate-600 mb-0.5">▸ {l.name} — {l.level}</p>)}
            </section>
          )}
          {hobbies.length > 0 && (
            <section>
              <h2 className="text-xs font-black uppercase tracking-widest mb-2" style={{ color: accent }}>{SECTION_TITLES.hobbies}</h2>
